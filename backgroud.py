...
source_path = './U12M7.mp4'
#
# output_path to save tracking data
#output_path = 'U12M13_track.csv'
#
...
while True:
    ...
    if wait_time == 0:
        if k == 194:
            ...
            cv2.putText(roi, "click on the screen then press 'F5' to reset tracker.", (50, 100), font, scale, color,
                        thick, cv2.LINE_AA)
            cv2.putText(roi, "OR press 'F6' to continue tracking to next frame.", (50, 125), font, scale, color, thick,
                        cv2.LINE_AA)
            cv2.putText(roi, "OR press 'F7' to continue tracking on play.", (50, 150), font, scale, color, thick,
                        cv2.LINE_AA)
        else:
            cv2.putText(roi, "TRACKER IS PAUSED", (50, 50), font, scale, color, thick, cv2.LINE_AA)
            cv2.putText(roi, "click on the screen then press 'F5' to reset tracker.", (50, 75), font, scale, color,
                        thick, cv2.LINE_AA)
            cv2.putText(roi, "OR press 'F6' to continue tracking the next frame.", (50, 100), font, scale, color, thick,
                        cv2.LINE_AA)
            cv2.putText(roi, "OR press 'F7' to continue tracking on play.", (50, 125), font, scale, color, thick,
                        cv2.LINE_AA)
    else:
